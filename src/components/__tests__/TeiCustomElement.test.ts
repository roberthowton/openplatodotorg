...
import { parseDatasetConfig, applyTeiConfig, createTeiCustomElement } from '../TeiCustomElement';
...
describe('createTeiCustomElement', () => {
  ...
  it('returned class has connectedCallback method', () => {
    const TeiClass = createTeiCustomElement();
    expect(typeof TeiClass.prototype.connectedCallback).toBe('function');
  });
  ...
  it('connectedCallback calls parseDatasetConfig and applyTeiConfig', () => {
    ...
    const TeiClass = createTeiCustomElement();
    const element = document.createElement('div') as HTMLElement & { connectedCallback: () => void };
    element.innerHTML = '<div>TEI content</div>';
