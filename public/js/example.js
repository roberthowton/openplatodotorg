$(document).ready(function () {
  ...
  $(".hl").hover(function (e) {
    ...
    englishid = "#e" + hlid;
    if ($(this).attr("id")[0] == "g") {
      $(englishid).toggleClass("hovered");
    }
    if ($(this).attr("id")[0] == "e") {
      $(greekid).toggleClass("hovered");
    }
  });
  cssGE = {
  ...
  $("#nav-greek").click(function (e) {
    if ($("#greek:visible").length == 1) {
      ...
      if ($("#comments:visible").length == 1) {
        //no greek, english, comments
        $("article").css(cssEC);
        if ($("#comments > p:visible").length == 1) {
          //adjust comment offset
  ...
  $("#nav-frs").click(function (e) {
    ...
    if (!$(this).hasClass("current-view")) {
      //store scholarly and load fr
      $("article").append(
        '<section id="english-tmp" style="display:none;">' +
          $("#english").html() +
          "</section>",
      );
      $("article").append(
        '<section id="comments-tmp" style="display:none;">' +
          $("#comments").html() +
          "</section>",
      );
